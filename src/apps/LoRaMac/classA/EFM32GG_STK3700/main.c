...
/*!
 * Timer to handle the state of LED0
 */
static TimerEvent_t Led0Timer;
 ...
 */
static void OnLed0TimerEvent( void )
{
    TimerStop( &Led0Timer );
    // Switch LED 0 OFF
    GpioWrite( &Led0, 0 );
...
//                GpioWrite( &Led2, ( AppLedStateOn & 0x01 ) != 0);
            }
            break;
        case 224:
    ...
    while( 1 )
    {
        switch( DeviceState )
        {
            case DEVICE_STATE_INIT:
            {
                ...
                TimerInit( &Led0Timer, OnLed0TimerEvent );
                TimerSetValue( &Led0Timer, 100 );
